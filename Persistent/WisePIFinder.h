...
const int TimeWindosPara = 1500; 
const uint8_t persitTimeWindows = 0.4*TimeWindosPara; 
...
// WisePIFinder::WisePIFinder(int threshold1, double threshold2, int maxBucketNum, int K) {
//     _threshold1 = threshold1;
//     _threshold2 = threshold2;
//     _K = K;
//     _bobHash = new BOBHash64(1005);
    ...
    if(!is_insert){ 
        ...
        else
        {
            ...
            if (lossy_entry0.count < lossy_entry1.count && lossy_entry0.count >= 1){
                lossy_entry0.count -= 0.001;
                if(lossy_entry0.count < 0.5)
                {
                    //_buckets[0][keys[0]].entries[3].count = 0;
                    lossy_entry0.count = 0;
                }
            }else if(lossy_entry1.count < lossy_entry0.count && lossy_entry1.count >= 1){
                lossy_entry1.count -= 0.001;
                if(lossy_entry1.count < 0.5)
                {
                    lossy_entry1.count = 0;
                }
            }
