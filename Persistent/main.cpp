...
enum class func_type {
    ASKETCH, WisePIFinder, CUCKOOCOUNTER, ELASTIC, HEAVYKEEPER, LOSSYCOUNTING, MVSKETCH, NITROSKETCH, SPACESAVING
};

// const std::vector<std::string> func_names = {
//     "ASketch", "WisePIFinder", "CuckooCounter", "ElasticSketch", "HeavyKeeper", "LossyCounting", "MVSketch", "NitroSketch", "SpaceSaving"
// };
...
int main(int argc, char** argv)
{
    ...
    //piflow: fp(ID), N, flag, C, E, m = 4 + 2 + 1 + 2 + 2 = 11
    //persistent: fp(ID), N, flag, C, E = 4 + 2 + 2 + 2 = 10
    // preparing WisePIFinder
    int CS_M;
    for (CS_M = 1;  ((2+ 3* MAX_ENTRY + 4) * CC_d)  * CS_M  <= MEM * 1024; CS_M++); if (CS_M % 2 == 0) CS_M--;
    // return 0;
    // int CS_M;
    // #ifdef CHECK_TOP
    ...
    std::cout << "WisePIFinder maxBucketNum: " << CS_M * CC_d << std::endl;
    // return 0;
    // OnOffArray = new int[2 * CS_M * MAX_ENTRY];
    //std::cout << "WisePIFinder OnOffArray: " << sizeof(OnOffArray)/sizeof(OnOffArray[0]) << std::endl;
    // func.push_back(new WisePIFinder(250, 0.5, CS_M, K));
    // CS_M <<= 1;
    // CS_M--;
    func.push_back(new WisePIFinder(125, 0.2, CS_M, K));
    ...
    printf("\n*************prepare Persistent flow************\n");
    //std::cout << "Time Windows Max num: " << (m-1)/TimeWindosPara + 1<< std::endl;
    std::cout << "Max Windows: " << TimeWindosPara << std::endl;
    ...
    for (int i = 0; i < m; i++)
   {
    ...
    std::cout << "Persistent flow size " <<  PIflow.size() << std :: endl; 
    ...
    for (auto &sketch_func : func) {
        // if (sketch_func->get_name() == "WisePIFinder") continue;
    ...
    for (auto &sketch_func : func) {
        ...
        for (int i = 0; i < res_count; i++) {
            ...
            // if (sketch_func->get_name() == "WisePIFinder") {
            //     if (B[str] != num) {
