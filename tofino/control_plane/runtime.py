...
class bfrt_runtime():
    ...
    def reg_read(self, reg_name, index):
        ...
        value = data_dict[reg_name + ".f1"][1]
        return value

    def reg_read_all(self, reg_name, bucket_num):
        '''
        read ALL blocks' values of a register (Dump entire table)
        Args: 
            - reg_name: entire register name, e.g. SwitchIngress.control_block_name.reg_name
            - bucket_num: total size of the register (default 65536)
        return a list of int values (length = bucket_num)
        '''

        self.register = self.bfrt_info.table_get(reg_name)
        
        snap = self.register.entry_get(
            self.target, 
            None, 
            {"from_hw": True}
        )
        
        all_values = [0] * bucket_num
        
        for data, key in snap:
            data_dict = data.to_dict()
            key_dict = key.to_dict()
            
            idx = key_dict['$REGISTER_INDEX']['value']
            
            val = data_dict[reg_name + ".f1"][1]
